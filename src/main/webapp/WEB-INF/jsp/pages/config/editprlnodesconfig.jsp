...
<body>
 	 <table class="tblHeader-inner">																																						
		<tr>
			<td colspan="2" align="center"><font size="3"><b>Edit Parallel Nodes Configuration</b></font><br />
			</td>
		</tr>
	...
	<table id="editobjspyconfig" class="table table-striped table-bordered table-condensed table-hover">
		<thead>
			<tr>
				<th colspan="1">Node Path</th>
				<th colspan="0">Ip Address</th>
				<th colspan="0">Platform</th>
				
				<!-- <th colspan="2">Assigned Projects</th> -->
			</tr>
		</thead>
		<tbody>
			<c:if test="${not empty prlNodes_list}">
				<c:forEach items="${prlNodes_list}" var="item">
					<tr>
						<td colspan="1"><a href="prlnodes/${item.id}/updateprlnodes">${item.nodeurl}</a><br/>
						...
						<td colspan="0">${item.ipAddress}</td>
						<td colspan="0">${item.platform}</td>
					</tr>
				</c:forEach>
			</c:if>
		</tbody>
	...
	<c:if test="${fn:length(prlNodes_list)>10}">
