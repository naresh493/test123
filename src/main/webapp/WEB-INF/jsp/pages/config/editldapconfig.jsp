...
<body>
 	 <table class="tblHeader-inner">																																						
		<tr>
			<td colspan="2" align="center"><font size="3"><b>Edit Ldap Configuration</b></font><br />
			</td>
		</tr>
	...
	<div>
	<table id="editldapconfig" class="table table-striped table-bordered table-condensed table-hover">
		<thead>
			<tr>
				<th colspan="1">Name</th>
				<th colspan="0">Server Ip</th>
				<th colspan="0">Port</th>
				<th colspan="0">UserName</th>
		...
		<tbody>
			<c:if test="${not empty ldapconfig_list}">
				<c:forEach items="${ldapconfig_list}" var="item">
					<tr>
						<td colspan="1"><a href="ldapconfig/${item.id}/updateldap">${item.name}</a><br/>
						...
						<td colspan="0">${item.serverip}</td>
						<td colspan="0">${item.port}</td>
						<td colspan="0">${item.username}</td>
					</tr>
				</c:forEach>
			</c:if>
		</tbody>
	...
	<script type="text/javascript">
		$(document).ready(function(){
  			$("table#editldapconfig").tablesorter({widthFixed: true, widgets: ['zebra']})
